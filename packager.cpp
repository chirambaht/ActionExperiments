 ...
 * @constructor
 */
ActionTracer::Communication::Supervisor::Supervisor( uint16_t port ) { set_server_port( port ); }

/**
 * @brief Construct a new Action Tracer:: Supervisor:: Supervisor object
 * @constructor
 */
ActionTracer::Communication::Supervisor::Supervisor() { set_server_port( 9022 ); }
...
int ActionTracer::Communication::Supervisor::_socket_setup() {
	...
	// This helps in manipulating options for the socket referred by the socket descriptor sockfd. This is completely
	...
	if( bind( _server.get_descriptor(), ( struct sockaddr * ) &_server._server_details,
			sizeof( _server.get_details() ) ) < 0 ) {
		printf( "bind failed" );
		exit( EXIT_FAILURE );
	}
...
/**
 * @brief Manages the TCP server by adding clients to Supervisors network. It accepts a client and obtains the ncessary
 *information to create a recepient.
...
uint8_t ActionTracer::Communication::Supervisor::_wait_for_connection() {
	...
	temp_client->set_descriptor( accept( _server.get_descriptor(), ( sockaddr * ) &temp_client->address,
		&temp_client->_address_len ) ); // Blocking call waiting for new connection
...
/**
 * This is used to send the stored data packet to all clients connected. This will first load the data packet into a
 * buffer and then send it to all clients.
 * @return Nothing
...
int ActionTracer::Communication::Supervisor::load_packet( ActionDataPackage *device_packet ) {
	...
	ActionDataNetworkPackage_ActionDeviceData_ActionData3 *accelerometer =
		new ActionDataNetworkPackage_ActionDeviceData_ActionData3();
	ActionDataNetworkPackage_ActionDeviceData_ActionData3 *gyroscope =
		new ActionDataNetworkPackage_ActionDeviceData_ActionData3();
 ...
 * @returns Nothing
 */
void ActionTracer::Communication::Supervisor::set_server_port( const uint16_t port ) { _server.set_port( port ); }
 ...
 * @returns Nothing
 */
uint16_t ActionTracer::Communication::Supervisor::get_server_port() const { return _server.get_port(); }
 ...
 * @returns Nothing
 */
void ActionTracer::Communication::Supervisor::set_ready( bool status ) { _ready = status; }
 ...
 */
ActionTracer::Communication::ActionServer::ActionServer( sockaddr_in my_address, uint16_t port )
	: _port( port ), address( my_address ) {}
 ...
 */
socklen_t ActionTracer::Communication::ActionServer::get_socket_address_length() const { return _address_len; }
 ...
 */
size_t ActionTracer::Communication::ActionServer::get_clients_connected() const { return _clients.size(); }
 ...
 * @returns Nothing
 */
void ActionTracer::Communication::ActionServer::set_address( const sockaddr_in my_address ) { address = my_address; }

/**
 * @brief Obtains the network address of the device
 * @returns _ready The ready status of the device
 */
sockaddr_in ActionTracer::Communication::ActionServer::get_address() const { return address; }

/**
 * @brief Gets the port of the device
 * @returns uint16_t The port of the device
 */
uint16_t ActionTracer::Communication::ActionServer::get_port() const { return _port; }
 ...
 * @returns Nothing
 */
void ActionTracer::Communication::ActionServer::set_port( const uint16_t port ) { _port = port; }
 ...
 * @returns Nothing
 */
void ActionTracer::Communication::ActionServer::set_descriptor( const int descriptor ) { _descriptor = descriptor; }
 ...
 */
ActionTracer::Communication::ActionServerClient::ActionServerClient( sockaddr_in client_address, uint8_t descriptor )
	: _descriptor( descriptor ), address( client_address ) {}
 ...
 */
socklen_t ActionTracer::Communication::ActionServerClient::get_socket_address_length() const { return _address_len; }

/**
 * @brief Gets the descriptor of the client
 * @returns int The descriptor of the client
 */
uint8_t ActionTracer::Communication::ActionServerClient::get_descriptor() const { return _descriptor; }
...
/**
 * @brief Sends the disconnect notification to the client. This notification is a standard message with no and packet
 * number set to 0 and as no data.
 * @returns Nothing
 */
